...
async function lintWiki() {
  try {
    ...
    // تتبع الروابط الواردة لكشف الصفحات اليتيمة
    ...
    for (const filePath of allFiles) {
      ...
      // فحص الروابط الداخلية [[اسم الصفحة]]
      const linkRegex = /\[\[([^\|\]#]+)(?:[\|#][^\]]+)?\]\]/g;
      let match;
      ...
      // فحص التناقضات والتحذيرات
      if (content.includes("> [!WARNING]")) {
        warningsCount++;
      }
    }
    
    // تحديد الصفحات اليتيمة (التي ليس لها روابط واردة، باستثناء صفحة النظرة العامة)
