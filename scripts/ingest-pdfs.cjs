...
const parsePDF = typeof pdfParser === 'function' ? pdfParser : pdfParser.default;
...
async function start() {
  ...
  for (const target of TARGET_FILES) {
    ...
    try {
      ...
      const PDFParse = pdfParser.PDFParse || pdfParser.default || pdfParser;
      const parser = new PDFParse({ data: dataBuffer, verbosity: 0 });
      const result = await parser.getText();
      const fullText = result.text || "";
      const pageCount = Number(result.total || result.numpages || 0);
      
      console.log(`تم قراءة الملف بنجاح. عدد الصفحات: ${pageCount}. حجم النص: ${fullText.length} حرف.`);
      
      const rawChunks = createChunks(fullText);
      console.log(`تم توليد ${rawChunks.length} مقطع مبدئي.`);
      
      const maxChunks = Math.min(rawChunks.length, 50); 
      const selectedChunks = rawChunks.slice(0, maxChunks);
  ...
  try {
    ...
    console.log(`إجمالي العناصر في الموسوعة المحلية الآن: ${localItems.length} عنصر.`);
  } catch (err) {
